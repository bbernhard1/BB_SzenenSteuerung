...
class BB_SzenenSteuerung extends IPSModule
{
    public function Create()
    {
        ...
        $this->RegisterAttributeInteger('ActiveScene', 1);
        ...
        if (!IPS_VariableProfileExists('SZS.SceneControlExecute')) {
            IPS_CreateVariableProfile('SZS.SceneControlExecute', 1);
            IPS_SetVariableProfileValues('SZS.SceneControlExecute', 2,2, 0);
            //IPS_SetVariableProfileIcon("SZS.SceneControl", "");
            IPS_SetVariableProfileAssociation('SZS.SceneControlExecute', 2, $this->Translate('Execute'), '', -1);
        }


        if(!IPS_VariableProfileExists('SZS.SceneLock')) {
            IPS_CreateVariableProfile('SZS.SceneLock', 0);
            IPS_SetVariableProfileValues('SZS.SceneLock', false, true, 0);
            IPS_SetVariableProfileAssociation('SZS.SceneLock', false, $this->Translate('Open'), '', 0xFF0000);
            IPS_SetVariableProfileAssociation('SZS.SceneLock', true, $this->Translate('Locked'), '', -1);
        }
    }
    ...
    public function ApplyChanges()
    {
        ...
        $this->RegisterVariableBoolean('SceneLock',$this->translate('SceneLock'),'SZS.SceneLock');
        $this->EnableAction('SceneLock');
    ...
    {
        // Modify variable profile 
        if ($Ident == 'SceneLock') {

            $this->SetValue('SceneLock', $Value);
            for ($i = 1; $i <= $this->ReadPropertyInteger('SceneCount'); $i++) {
                $sceneDataID = @$this->GetIDForIdent('Scene' . $i);
                
                if ($Value == true) {           // Profile without save button
                    IPS_SetVariableCustomProfile($sceneDataID, 'SZS.SceneControlExecute');
                }
                else {                          // Profile with  save & execute button   
                    IPS_SetVariableCustomProfile($sceneDataID, 'SZS.SceneControl');
                } 
            }
    ...
    public function CallScene(int $SceneNumber)
    {
        ...
        $this->WriteAttributeInteger('ActiveScene', $SceneNumber); 
    }
    ...
    {
            $ActiveScene = intval($this->ReadAttributeInteger('ActiveScene')) + 1;  
            if ($ActiveScene > $this->ReadPropertyInteger('SceneCount'))
            {
                 $ActiveScene = 1;
            }    
            $this->CallScene($ActiveScene);
    } 
